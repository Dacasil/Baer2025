...
class Comparison:
    def __init__(self, account, profile, passport):
        ...
        # png
        date_of_birth_value = passport.loc[
            passport["Field"] == "date_of_birth", "Value"
        ].iloc[0]

        self.passport = passport
        self.passport["Field"] = (
            self.passport["Field"].str.lower().str.replace(" ", "", regex=False)
        )
        # self.passport = passport.set_index("Field").T
        try:
            self.passport.loc[self.passport["Field"] == "date_of_birth", "Value"] = (
                pd.to_datetime(
                    self.passport.loc[
                        self.passport["Field"] == "date_of_birth", "Value"
                    ],
                    format="%d%m%Y",
                )
            )
        ...
        try:
            self.passport.loc[self.passport["Field"] == "issue_date", "Value"] = (
                pd.to_datetime(
                    self.passport.loc[self.passport["Field"] == "issue_date", "Value"],
                    format="%d%m%Y",
                )
        ...
        try:
            self.passport.loc[self.passport["Field"] == "expiration_date", "Value"] = (
                pd.to_datetime(
                    self.passport.loc[
                        self.passport["Field"] == "expiration_date", "Value"
                    ],
                    format="%d%m%Y",
                )
            )
        ...
        self.profile["Field"] = (
            self.profile["Field"].str.lower().str.replace(" ", "", regex=False)
        )
        try:
            self.profile.loc[self.profile["Field"] == "dateofbirth", "Value"] = (
                pd.to_datetime(
                    self.profile.loc[self.profile["Field"] == "dateofbirth", "Value"],
                    format="%Y-%m-%d",
                )
        ...
        try:
            self.profile.loc[self.profile["Field"] == "idissuedate", "Value"] = (
                pd.to_datetime(
                    self.profile.loc[self.profile["Field"] == "idissuedate", "Value"],
                    format="%Y-%m-%d",
                )
        ...
        try:
            self.profile.loc[self.profile["Field"] == "idexpirydate", "Value"] = (
                pd.to_datetime(
                    self.profile.loc[self.profile["Field"] == "idexpirydate", "Value"],
                    format="%Y-%m-%d",
                )
        ...
        self.account["Field"] = (
            self.account["Field"].str.lower().str.replace(" ", "", regex=False)
        )
    ...
    def findField(self, nameVersions):
        ...
        for name in nameVersions:
            matchesProfile.extend(
                self.profile[self.profile["Field"] == name].values[:, 1]
            )
            matchesPassport.extend(
                self.passport[self.passport["Field"] == name].values[:, 1]
            )
            matchesAccount.extend(
                self.account[self.account["Field"] == name].values[:, 1]
            )
    ...
    def compare(self, nameVersions):
        ...
        names = {"profile": False, "passport": False, "account": False}
        for name1, val1 in matches.items():
            for name2, val2 in matches.items():
                ...
                for val in val1:
                    try:
                        if val.lower().replace(" ", "") in [
                            v.lower().replace(" ", "") for v in val2
                        ]:
                            names[name1] = True
                    except:
...
def comparePrecise(account="", profile="", passport=""):
    ...
    answer = Client.CrossMatchingProperties()
